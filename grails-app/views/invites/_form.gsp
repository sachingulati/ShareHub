...
<div class="fieldcontain ${hasErrors(bean: invitesInstance, field: 'invitedBy', 'error')} required">
	...
	<g:select id="invitedBy" name="invitedBy.id" from="${sharehub.User.list()}" optionKey="id" required="" value="${invitesInstance?.invitedBy?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: invitesInstance, field: 'invitedTo', 'error')} required">
	...
	<g:select id="invitedTo" name="invitedTo.id" from="${sharehub.User.list()}" optionKey="id" required="" value="${invitesInstance?.invitedTo?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: invitesInstance, field: 'status', 'error')} required">
	...
	<g:select name="status" from="${com.sharehub.enums.InviteStatus?.values()}" keys="${com.sharehub.enums.InviteStatus.values()*.name()}" required="" value="${invitesInstance?.status?.name()}" />
...
<div class="fieldcontain ${hasErrors(bean: invitesInstance, field: 'topic', 'error')} required">
	...
	<g:select id="topic" name="topic.id" from="${sharehub.Topic.list()}" optionKey="id" required="" value="${invitesInstance?.topic?.id}" class="many-to-one"/>
