...
<div class="fieldcontain ${hasErrors(bean: resourceStatusInstance, field: 'resource', 'error')} required">
    ...
    <g:select id="resource" name="resource.id" from="${sharehub.Resource.list()}" optionKey="id" required=""
              value="${resourceStatusInstance?.resource?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: resourceStatusInstance, field: 'user', 'error')} required">
    ...
    <g:select id="user" name="user.id" from="${sharehub.User.list()}" optionKey="id" required=""
              value="${resourceStatusInstance?.user?.id}" class="many-to-one"/>
