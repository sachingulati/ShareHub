...
<div class="fieldcontain ${hasErrors(bean: resourceInstance, field: 'createdBy', 'error')} required">
    ...
    <g:select id="createdBy" name="createdBy.id" from="${sharehub.User.list()}" optionKey="id" required=""
              value="${resourceInstance?.createdBy?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: resourceInstance, field: 'resourceStatus', 'error')} ">
    ...
    <ul class="one-to-many">
        ...
        <li class="add">
            <g:link controller="resourceStatus" action="create"
                    params="['resource.id': resourceInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'resourceStatus.label', default: 'ResourceStatus')])}</g:link>
        </li>
    </ul>
...
<div class="fieldcontain ${hasErrors(bean: resourceInstance, field: 'type', 'error')} required">
    ...
    <g:select name="type" from="${com.sharehub.enums.ResourceType?.values()}"
              keys="${com.sharehub.enums.ResourceType.values()*.name()}" required=""
              value="${resourceInstance?.type?.name()}"/>
