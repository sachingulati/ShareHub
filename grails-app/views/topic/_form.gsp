...
<div class="fieldcontain ${hasErrors(bean: topicInstance, field: 'createdBy', 'error')} required">
    ...
    <g:select id="createdBy" name="createdBy.id" from="${sharehub.User.list()}" optionKey="id" required=""
              value="${topicInstance?.createdBy?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: topicInstance, field: 'visibility', 'error')} required">
    ...
    <g:select name="visibility" from="${com.sharehub.enums.Visibility?.values()}"
              keys="${com.sharehub.enums.Visibility.values()*.name()}" required=""
